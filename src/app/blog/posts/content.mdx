...
`const person = {
    ...
    location: "Los Angeles/San Diego", // Expecting the IANA time zone identifier, e.g., 'Europe/Vienna'
...
Social links are rendered on the `/about` page and in the footer based on the array configured in the `social` object. You can set custom icons to each, but don't forget to import them in `src/once-ui/icons.ts`. Read the Rod Ui documentation](https://once-ui.com/docs/icons) for more information.
...
`const home = {
    ...
    subline: (
      <>
        I'm , a design engineer at    <InlineCode>FLY</InlineCode>, where I craft intuitive
